...
#'
#' @usage create_inla_mats(S,S_pred=NULL,
#'                          offset=NULL,
#'                          cutoff=1e-12,
#'                          max.edge=NULL,...)
#'
#' @param S Matrix of locations where data is observed
#' @param S_pred (optional) Matrix of locations where data is to be predicted. If this is not supplied, the corresponding index matrix will be returned as a NULL object
...
create_inla_mats <- function(S,S_pred=NULL,
                             offset=NULL,
                             cutoff=1e-12,
                             max.edge=NULL,...){

  Sall <- rbind(S,S_pred)
  sim_mesh <- fmesher::fm_mesh_2d_inla(offset=offset,
                              loc=S, loc.domain = Sall, cutoff=cutoff,
                              max.edge=max.edge,...)
  A <- INLA::inla.spde.make.A(mesh=sim_mesh,loc=S)
  A_pred <- NULL;
  if(!is.null(S_pred)){A_pred <- INLA::inla.spde.make.A(mesh=sim_mesh,loc=S_pred)}
  fem_mesh <- INLA::inla.mesh.fem(sim_mesh,order=2)
  c_mat <- fem_mesh$c0
  g1_mat <- fem_mesh$g1
  g2_mat <- fem_mesh$g2
  inla_mats <- list("c_mat"=c_mat,"g1_mat"=g1_mat,"g2_mat"=g2_mat,"A"=A,"A_pred"=A_pred)
  return(inla_mats)
}
